...
CREATE TABLE `user_money` (
  ...
  PRIMARY KEY (`id`),
  CONSTRAINT `user_money` FOREIGN KEY (`user_id`) REFERENCES `user` (`id`)
...
```sql
ALTER TABLE `fk_account_to_user` ADD CONSTRAINT `account` FOREIGN KEY (`user_id`) REFERENCES `user` (`id`);
```
