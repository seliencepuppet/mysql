MySQL 用户密码修改：
```shell
[root@zhangyz ~]# mysqladmin -u USERNAME -H HOSTNAME password 'new_password' -p
[root@zhangyz ~]# mysql
mysql> SET PASSWORD FRO 'USERNAME'@'HOST'=PASSWORD('new_pass');
...
1.使用cmake编译mysql-5.5 cmake 指定编译选项的方式不同于make， 实现方式对比如下：
```shell
./configure         cmake . 
...
2.编译安装
```shell
[root@zhangyz ~]# groupadd -r mysql
[root@zhangyz ~]# useradd -g mysql -r -d /data/mydata mysql
[root@zhangyz ~]# tar xf mysql-5.5.37.tar.gz
[root@zhangyz ~]# cd mysql-5.5.37
[root@zhangyz mysql-5.5.37]# cmake . -DCMAKE_INSTALL_PREFIX=/usr/local/mysql \
...
-DDEFAULT_CHARSET=utf8 \
-DDEFAULT_COLLATION=utf8_general_ci
[root@zhangyz mysql-5.5.37]# make
[root@zhangyz mysql-5.5.37]# make install
```
-------------------------------------------------------------------
